...
/**
 * Job de synchronisation Majliss → HumHub via API REST WordPress.
 *
 * Exécuté via le système de queue HumHub,
 * déclenché par le cron horaire ou manuellement.
 *
 * Architecture : utilise l'API REST WP (pas de connexion DB directe).
 */
class MajlissSyncJob extends ActiveJob
{
    ...
    public function run()
    {
        try {
            ...
            Yii::info(
                "MajlissSyncJob (API REST): {$result['success']} succès, {$result['errors']} erreur(s).",
