 ...
 *
 * IMPORTANT : le champ texte est nommé 'post_content' (pas 'content')
 * car ContentActiveRecord utilise getContent() pour sa relation Content.
 * Un champ DB 'content' masquerait cette relation.
 *
 * @property int         $id
 * @property int         $wp_post_id       ID du post dans WordPress
 * @property string      $title            Titre du post
 * @property string|null $post_content     Contenu texte nettoyé (pas 'content' !)
...
class MajlissPost extends ContentActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['post_content', 'sync_error'], 'string'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'post_content'    => Yii::t('ExternalHtmlStreamModule.base', 'Contenu'),
    ...
    /**
     * Supprime complètement le post et son content associé.
     */
    public function hardDelete(): bool
    {
        try {
            ...
            // Si le content est corrompu, forcer la suppression
            Yii::$app->db->createCommand()
                ->delete('majliss_synced_post', ['id' => $this->id])
                ->execute();
            return true;
        }
    }
     ...
     * Utilisé par le backoffice et les exports.
     */
    public function getFormattedContent(): string
    {
        $dir = $this->getTextDirection();
        $html = '<div dir="' . $dir . '">';
        ...
        if (!empty($this->category)) {
            $meta .= '<span class="label label-primary" style="margin-' . ($dir === 'rtl' ? 'left' : 'right') . ': 6px;">'
        ...
        // Contenu HTML riche — affiché tel quel (déjà sanitisé par HTMLPurifier)
        $html .= '<div class="majliss-post-text majliss-rich-html">' . ($this->post_content ?? '') . '</div>';
    ...
    /**
     * Détecte si le contenu est principalement en arabe/hébreu (RTL).
     *
     * Analyse le titre et le contenu pour détecter les caractères
     * des scripts RTL (arabe, hébreu, farsi, ourdou, etc.)
     *
     * @return bool true si le contenu doit être affiché en RTL
     */
    public function isRtl(): bool
    {
        $text = strip_tags($this->title . ' ' . ($this->post_content ?? ''));

        // Compter les caractères RTL (arabe, hébreu, farsi, syriaque, thaana)
        $rtlCount = preg_match_all('/[\x{0600}-\x{06FF}\x{0750}-\x{077F}\x{08A0}-\x{08FF}\x{FB50}-\x{FDFF}\x{FE70}-\x{FEFF}\x{0590}-\x{05FF}\x{0700}-\x{074F}]/u', $text);

        // Compter les caractères LTR (latin, cyrillique, etc.)
        $ltrCount = preg_match_all('/[a-zA-Z\x{00C0}-\x{024F}\x{0400}-\x{04FF}]/u', $text);

        // Si > 40% des caractères alphabétiques sont RTL → RTL
        $total = $rtlCount + $ltrCount;
        if ($total === 0) {
            return false;
        }

        return ($rtlCount / $total) > 0.4;
    }
