 ...
 *
 * Gère :
 *  - Le toggle "Afficher plus / Réduire" des posts Majliss
...
/**
 * Toggle "Afficher plus / Réduire" pour les posts Majliss.
 * Appelé directement depuis onclick dans la vue.
 *
 * @param {string} id   ID unique du post (majliss-post-{id})
 * @param {boolean} show  true = afficher tout, false = réduire
 */
function majlissToggle(id, show) {
    ...
    if (show) {
        ...
        // Smooth scroll vers le haut du post
        ...
        // Scroll vers le haut du post au moment de réduire
        var card = document.getElementById(id);
        if (card) {
            card.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
...
(function ($) {
    ...
    var ExternalHtmlStream = {
        ...
        bindEvents: function () {
            ...
            // Observer les nouveaux contenus dans le stream (scroll infini)
