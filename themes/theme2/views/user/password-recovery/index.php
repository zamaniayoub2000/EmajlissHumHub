...
.l-bg-logo{
  ...
  pointer-events:none;z-index:0;opacity:.00;
  filter:brightness(0) saturate(100%) invert(22%) sepia(60%) saturate(700%) hue-rotate(320deg) brightness(0.85);
...
.l-deco-slash::before{
  ...
  width:90px;height:1px;background:rgba(165,53,53,0.07);
}
.l-deco-slash::after{
  content:'';position:absolute;top:14px;left:20px;
  width:70px;height:1px;background:rgba(165,53,53,0.05);
}
.l-deco-dot{
...
.l-site-name{
  ...
  text-align:center;
  text-shadow:0 1px 3px rgba(0,0,0,0.18), 0 2px 8px rgba(0,0,0,0.09);
}

/* ── CARD ── */
.l-card-wrap{
  width:100%;max-width:min(420px,88vw);
  position:relative;z-index:2;
}

.l-card-wrap .panel.panel-default{
  ...
  opacity:0;
  animation:slideUp 1.1s cubic-bezier(.16,1,.3,1) .2s forwards;
  transition:border-color .3s, box-shadow .35s, transform .35s cubic-bezier(.16,1,.3,1);
}
.l-card-wrap .panel.panel-default:hover{
...
.l-card-wrap .panel.panel-default .panel-heading{position:relative;overflow:hidden;}
.l-card-wrap .panel.panel-default .panel-heading::after{
...
.l-card-wrap .panel.panel-default:hover .panel-heading::after{transform:translateX(0);}

/* ── PANEL HEADING ── */
.l-card-wrap .panel.panel-default .panel-heading{
...
/* ── PANEL BODY ── */
.l-card-wrap .panel.panel-default .panel-body{
...
.l-card-wrap .form-control::placeholder{color:var(--text3)!important;font-size:13px!important;}
.l-card-wrap .form-control:focus{
  border-color:var(--crimson)!important;
  background:var(--input-focus)!important;
  box-shadow:0 0 0 3.5px var(--glow)!important;
}
.l-card-wrap .form-group{margin-bottom:16px!important;}

/* ── CAPTCHA ── */
.l-card-wrap .captcha-field{margin-bottom:16px;}

/* ── BUTTONS ROW ── */
.l-btn-row{display:flex;gap:10px;margin-top:4px;}

/* Back button */
.l-card-wrap .btn.btn-default,
.l-card-wrap .btn.btn-light{
  flex:0 0 auto;
...
.l-card-wrap .btn.btn-default:hover,
.l-card-wrap .btn.btn-light:hover{
...
.l-card-wrap .btn.btn-primary::before{
  content:'';position:absolute;top:0;left:-100%;width:60%;height:100%;
  background:linear-gradient(90deg, transparent, rgba(255,255,255,0.18), transparent);
  transform:skewX(-20deg);transition:left .4s ease;
}
.l-card-wrap .btn.btn-primary:hover::before{left:150%;}
.l-card-wrap .btn.btn-primary:hover{
  background:var(--crimson-h)!important;
  transform:translateY(-2px)!important;
  box-shadow:0 10px 28px var(--glow), 0 3px 8px rgba(0,0,0,0.1)!important;
}
.l-card-wrap .btn.btn-primary:active{transform:translateY(0)!important;}

/* ── ALERT ── */
.l-card-wrap .alert-danger{
...
@media(max-width:500px){
  #user-password-recovery{padding:32px 16px!important;}
  ...
  .l-btn-row{flex-direction:column;}
  .l-card-wrap .btn.btn-default,
  .l-card-wrap .btn.btn-light{width:100%!important;}
}
CSS);
?>
...
<div id="user-password-recovery">
  ...
  <div class="l-header">
    <div class="l-logo-wrap">
      <div class="l-logo-inner">
        <img src="/humhub/themes/theme2/views/user/img/logo.png" alt="CITOYENS CSEFRS">
      </div>
    </div>
  ...
  <!-- CARD -->
  <div class="l-card-wrap">
    <div class="panel panel-default" id="password-recovery-form">
      <div class="panel-heading">
        <div class="l-ph-title"><?= Yii::t('UserModule.auth', 'Password Recovery') ?></div>
        <div class="l-ph-sub"><?= Yii::t('UserModule.auth', "We'll send you recovery instructions right away.") ?></div>
      </div>
      <div class="panel-body">

        <div class="l-badge"><?= Yii::t('UserModule.auth', 'Account Recovery') ?></div>

        <p class="l-intro"><?= Yii::t('UserModule.auth', "Just enter your e-mail address. We'll send you recovery instructions!") ?></p>

        <?php $form = ActiveForm::begin(['enableClientValidation' => false]); ?>

        <label class="l-lbl"><?= Yii::t('UserModule.auth', 'Email Address') ?></label>
